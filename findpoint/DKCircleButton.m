...
#define DKCircleButtonBorderWidth 2.0f
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([touch locationInView:self].x>self.frame.size.width
        || [touch locationInView:self].x<0)
        isMove=true;
    if ([touch locationInView:self].y>self.frame.size.height|| [touch locationInView:self].y< 0 )
        isMove=true;
}
...
- (void)updateMaskToBounds:(CGRect)maskBounds {
    ...
    CGPoint point = CGPointMake(maskBounds.size.width, maskBounds.size.height);
    maskLayer.position = point;
    
//    [self.layer setMask:maskLayer];
    
    self.layer.cornerRadius = 8;//CGRectGetHeight(maskBounds) / 2.0;
    self.layer.borderColor = [self.borderColor colorWithAlphaComponent:0.7].CGColor;
